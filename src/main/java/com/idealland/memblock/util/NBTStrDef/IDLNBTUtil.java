package com.idealland.memblock.util.NBTStrDef;
...
import static com.idealland.memblock.util.IDLNBT.*;
...
public class IDLNBTUtil {
	...
	{
		int[] learnt = getPlayerIdeallandIntArraySafe(player, IDLNBTDef.STARTER_KIT_VERSION_TAG);
	...
	{
		int[] learnt = getPlayerIdeallandIntArraySafe(player, IDLNBTDef.LEARNING_DONE);
		...
		if (oldIndex >= 0)
		{
			...
			if (val)
			{
				...
				setPlayerIdeallandTagSafe(player, IDLNBTDef.LEARNING_DONE, newLearnt);
			}else {
				return;
			}
		}
